...
class _MasterCalendarScreenState extends State<MasterCalendarScreen> {
  ...
  Future<void> _onToggleGoogleSync(bool enable) async {
    if (!enable) {
      ...
      _toast(
        'Google sync off • removed $removed events',
        icon: Icons.link_off,
  ...
  Future<int> _purgeGoogleEventsFromMaster() async {
    ...
    while (true) {
      final page =
          await col.where('source', isEqualTo: 'google').limit(400).get();
  ...
  Future<void> _pickCalendarAndImport() async {
    ...
    preview.sort(
      (a, b) => (a['startTime'] as Timestamp).toDate().compareTo(
  ...
  Future<void> _syncNow() async {
    ...
    final existingSnap =
        await col
            ...
            .where(
              'startTime',
              isGreaterThanOrEqualTo: Timestamp.fromDate(win.from),
            )
  ...
  Map<String, dynamic>? _mapGoogleEventToLocalDoc(gcal.Event e) {
    ...
    return {
      ...
      'allDay': isAllDay, // unified field
      'isAllDay': isAllDay, // (optional) keep for back-compat
      'googleEventId': e.id,
      'iCalUID': e.iCalUID,
      'source': 'google',
      'createdAt': FieldValue.serverTimestamp(),
    ...
    final calendarCore = TableCalendar(
      ...
      calendarStyle: CalendarStyle(
        ...
        todayDecoration: const BoxDecoration(
          shape: BoxShape.circle,
        ).copyWith(color: cs.secondaryContainer),
  ...
  Widget build(BuildContext context) {
    ...
    final bg =
        _hover
            ...
            : Colors.transparent;
    final border =
        _hover ? accent.withOpacity(isDark ? 0.50 : 0.60) : Colors.transparent;
      ...
      builder:
          (_, controller) => Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Import from "${widget.calendarName}"',
                  style: Theme.of(context).textTheme.titleMedium,
                ),
                const SizedBox(height: 6),
                Text(
                  '${widget.events.length} events found (last 6 months → next 12 months)',
                ),
